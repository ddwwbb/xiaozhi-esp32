...
- **详情页**（列表页按 M）：固定单屏展示、不上下滚动；L/R 在当前数据源的账号间循环切换，M 返回列表。ChatGPT 账号显示邮箱、套餐、订阅有效期、5 小时/每周/代码审查窗口、重置倒计时、积分/连续天数/峰值摘要、累计 token 与刷新时间；智谱账号显示名称、套餐等级及 5h/周窗口，无数据项自动隐藏。
