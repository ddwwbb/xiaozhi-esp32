...
| L / R | 详情页内：切换账号（循环）；列表页：翻页（每页 2 个账号卡片）；否则音量 ± | 静音 / 最大音量 |

Q 键有效电平无原理图依据（L 键低有效、M 键高有效），上电按空闲电平自动探测；开机瞬间按住 Q 会导致极性学反，重启即恢复。

### 首次配置（长按 Q，浏览器打开屏幕显示的地址）

1. **导入 ChatGPT 令牌**：把 Codex 令牌文件内容（`~/.codex/auth.json`，或同格式导出 json）粘贴导入，需含 `refresh_token`。最多保存 4 个令牌；同一 ChatGPT 账号签发的不同令牌会分别保存，仅完全相同的令牌重复导入时覆盖，可单删。完整烧录新版分区表后，令牌使用独立 64KB `nvs_vendor` 分区；仅 OTA 升级的旧分区表设备会回退到默认 NVS，空间不足时导入会提示失败但不会重启。
...
- **官方 API 面板**（标题"AI 用量"，两功能都配置时按 Q 默认打开）：**分页显示，每页完整展示 2 张账号卡片**（240x320 屏，卡片弹性填满整页不留空白），页码在标题行右侧；L/R 翻页（循环）；标题行右侧同时显示数据更新时间。每账号一张卡片：名字 + 套餐徽章（Pro 紫/Team 黄/Free 灰/Plus 蓝）+ 每个窗口一行用量。窗口行为**单行**：`类型(重置时刻) + 进度条`，**百分比画在进度条内**（如 `5小时(14:35) ▓▓已用42%░░░`，重置时刻当天内显示 `hh:mm`、跨天显示 `dd日hh:mm`，依赖联网时间同步）。进度条为**已用百分比**（与 cc-switch 一致），颜色随用量变化：绿 <70%、黄 70–89%、红 ≥90%。窗口身份按官方返回的 `limit_window_seconds` 自动识别（5 小时/每周/30 天——免费与 ProLite 套餐的次窗口是 30 天而非每周），接口未返回的窗口行自动隐藏。智谱账号名字显示为 `智谱*xxxxxx`（Key 后 6 位），套餐徽章为官方返回的套餐等级。查询失败账号名字前有红色 `!`；任一数据源失败只影响自己的面板。
- **本机统计面板**（标题"本地用量"，导航行提示 Q 关闭；仅配置本机统计时按 Q 直接打开）：**每台 PC 一张卡片，一页显示 2 台**（L/R 翻页，4 台上限共 2 页），卡片布局与官方账号同套样式：标题行 = 名称（多 PC 时带地址尾段如 `本地@134`）+ 到午夜倒计时 + "本地"徽章，下接三行数据（主字体）：`日 0 ($0.00) · 0次` / `周 1242万 ($4.66) · 187次` / `累计 6.21亿 · $67.50`（本机统计无官方配额概念，不显示进度条）。**M 键进入该台 PC 的单屏详情页**：账号、当天/本周用量、模型分类前两项 + 其余汇总、累计 token/费用与更新时间均压缩在一屏内，不上下滚动。L/R 在 PC 间循环切换，M 返回列表。某台 PC 服务不可达时该台卡片红 `!`（详情提示"服务不可达，检查PC端服务"），其他 PC 与官方面板不受影响。统计范围：该 PC 上的 Claude Code + ZCode + Codex CLI（同名模型跨 CLI 合并）。
- **详情页**（列表页按 M）：标题行为 `账号详情 1/2`（页码紧跟标题）+ 右侧本次查询更新时间，无按键说明行。自上而下：**② 账号信息模块**（14px 小字体固定高度卡片：邮箱完整换行 + 套餐徽章 + 订阅至/积分/连续天数/峰值/累计用量/令牌刷新时间，无值字段隐藏，内容超高自动循环滚动）、**③ 窗口用量行**（固定行高单行：`类型 (重置时刻，当天 hh:mm/跨天 dd日hh:mm，14px 小字)` + 进度条内画 14px"已用xx%"，颜色绿 <70%/黄 70–89%/红 ≥90%，无数据的窗口不显示）、**⑤ 近 7 天每日用量柱状图**（柱下带日期标签，弹性吃满剩余空间；无图表数据的账号内容纵向均匀分布）。L/R 在当前数据源的账号间循环切换，M 返回列表。ChatGPT 账号显示邮箱、套餐徽章、5 小时/每周/30 天/代码审查窗口行、柱状图；智谱账号显示名称、套餐等级及 5h/周窗口行，无数据项自动隐藏。14px 小字库由 `lv_font_conv`（simhei）生成，新增字形见文件头命令。
...
| 端点 | 用途 |
|---|---|
| `GET chatgpt.com/backend-api/wham/usage` | 限额窗口（按 `limit_window_seconds` 识别 5h/每周/30 天）+ 代码审查窗口、套餐类型 |
...
TLS 证书走设备证书 bundle 校验，依赖系统时间正确（联网后自动同步）。时区固定为中国 UTC+8：小智服务端按出口 IP 定位时区，走代理时定位偏移会使设备时间快/慢 1 小时以上，固件忽略服务器下发的 `timezone_offset`（`main/ota.cc`）。
