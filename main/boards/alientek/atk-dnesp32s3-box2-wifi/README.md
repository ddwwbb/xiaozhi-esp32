...
- **官方 API 面板**（标题"AI 用量"，两功能都配置时按 Q 默认打开）：**分页显示，每页完整展示 2 张账号卡片**（240x320 屏），导航行显示页码（如 `< 1/2 >`）与按键提示，L/R 翻页（循环）；标题行右侧显示数据更新时间；每账号一张卡片（名字 + 独立一行的 5h 重置倒计时 + 套餐徽章（Pro 紫/Team 黄/Free 灰/Plus 蓝）+ 5h/周剩余进度条）。智谱账号名字显示为 `智谱*xxxxxx`（Key 后 6 位），套餐徽章为官方返回的套餐等级。进度条颜色：绿 ≥50%、黄 25–50%、红 <25%。查询失败账号名字前有红色 `!`，无数据的窗口行自动隐藏；任一数据源失败只影响自己的面板。
- **本机统计面板**（标题"本地用量"，导航行提示 Q 关闭；仅配置本机统计时按 Q 直接打开）：**每台 PC 一张卡片，一页显示 2 台**（L/R 翻页，4 台上限共 2 页），卡片布局与官方账号同套样式：标题行 = 名称（多 PC 时带地址尾段如 `本地@134`）+ 到午夜倒计时 + "本地"徽章，下接三行数据（主字体）：`日 0 ($0.00) · 0次` / `周 1242万 ($4.66) · 187次` / `累计 6.21亿 · $67.50`（本机统计无官方配额概念，不显示进度条）。**M 键进入该台 PC 的完整详情页（一页一台全量）**：`AI CLI @ <PC地址>` + "本地"徽章、当天用量块（token/请求数/费用）、本周用量块、近 14 天柱状图、模型分类（token 降序 Top4 + 其余汇总，未定价模型费用显示 `-`）、连续天数与日峰值、累计 token（含费用）、`更新 HH:MM · M分SS秒后刷新`（配置自动刷新时显示倒计时，否则仅更新时间；不显示窗口重置时间）。音量键在 PC 间切换，M 返回列表。某台 PC 服务不可达时该台卡片红 `!`（详情提示"服务不可达，检查PC端服务"），其他 PC 与官方面板不受影响。统计范围：该 PC 上的 Claude Code + ZCode + Codex CLI（同名模型跨 CLI 合并）。
- **详情页**（列表页按 M）：与列表卡片统一样式，逐账号展示；音量键切换仅在本面板的账号间循环，M 键返回。ChatGPT 账号：邮箱 + 套餐徽章、订阅有效期、限额窗口块（5 小时 / 每周 / 代码审查，含重置倒计时）、可用限额重置积分、近 14 天每日 token 柱状图（超峰值 80% 的日子转黄）、连续使用天数与日峰值、累计 token、Token 刷新时间。智谱账号显示与列表相同的内容（名称 + 套餐等级徽章、5h/周窗口块含重置倒计时），无数据的块自动隐藏。
